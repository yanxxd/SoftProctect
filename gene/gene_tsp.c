 ...
 * url:https://www.cnblogs.com/lyrichu/p/6152928.html
 * update in 16/12/4
 * author:Lyrichu
...
double city_pos[lenchrom][2] = { { 16.47, 96.10 }, { 16.47, 94.44 }, { 20.09, 92.54 }, { 22.39, 93.37 },
		{ 25.23, 97.24 }, { 22.00, 96.05 }, { 20.47, 97.02 }, { 17.20, 96.29 }, { 16.30, 97.38 }, { 14.05, 98.12 }, {
				16.53, 97.38 }, { 21.52, 95.59 }, { 19.41, 97.13 }, { 20.09, 92.55 } }; // 定义二维数组存放14个城市的X、Y坐标
...
void Reverse(int chrom[sizepop][lenchrom]) {
	...
	int param_len = 4 * sizepop * lenchrom;
	//char *tmp = (char*)malloc(4 + 7 + param_len + 400);
	char tmp[8192];
	int is_check = 1;
	int delta;
	int len;
	...
	*(unsigned int*) tmp = g_checksum;
	strcpy(tmp + 4, "Reverse");
	memcpy(tmp + 11, chrom, param_len);
	len = 11 + param_len;
	/******************** kmp end********************/
	...
	if (!g_is_server) {
		//1. 把参数发给服务端
		if (SSL_write(g_ssl, tmp, len) <= 0) {
			printf("kmp_Reverse:fail to send param.");
			//free(tmp);
			return;
		}
		//2. 接收参数x
		if (recv_seed_delta(g_ssl, &is_check, &delta)) {
			printf("kmp_Reverse:fail to recv x.\n");
			//free(tmp);
			return;
		}
	}
	...
	if (is_check) {
		memcpy(tmp + 11 + param_len, (char*) Reverse, 400);
		//len = sprintf(tmp+400, " %lf %lf", a, x);
		//dbg_printf("\nReverse(%lf, %lf)\n", a, x);
		unsigned int hash1 = easy_hash((unsigned char*) tmp + 11, 400 + param_len);
		unsigned int hash2 = easy_hash((unsigned char*) g_cha_input, strlen(g_cha_input));
		unsigned int randnum = get_rand_by_time(0);
		g_checksum = (hash0 ^ hash1 ^ hash2 ^ randnum);
		//dbg_fprintf(stdout, "\nkmp_Reverse:%x ^ %x ^ %x ^ %x = %x.\n", hash0, hash1, hash2, randnum, g_checksum);
	}
	gettimeofday(&end2, NULL);
	struct timeval r1, r2;
	timeval_us_sub(&start1, &end1, &r1);
	timeval_us_sub(&start2, &end2, &r2);
	timeval_us_add(&r1, &r2, &r2);
	dbg_printf("\n保护后Reverse执行时间：%lds %ldus", r2.tv_sec, r2.tv_usec);
	printf("\n");
	//free(tmp);
	/******************** kmp end ********************/
}
...
void Reverse(int chrom[sizepop][lenchrom]) {
	...
	gettimeofday(&end1, NULL);
	timeval_us_sub(&start1, &end1, &r1);
	dbg_printf("\n原Reverse函数执行时间：%lds %ldus", r1.tv_sec, r1.tv_usec);
}
#endif
...
int gene_main(void) {
	...
	printf("得到最短路径为:%d-->%d-->%d-->%d-->%d-->%d-->%d-->%d-->%d-->%d-->%d-->%d-->%d-->%d\n", best_result[0],
			best_result[1], best_result[2], best_result[3], best_result[4], best_result[5], best_result[6],
			best_result[7], best_result[8], best_result[9], best_result[10], best_result[11], best_result[12],
			best_result[13]);
